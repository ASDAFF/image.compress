...
class Compress
{
    ...
    private $MODULE_ID = 'image.compress';
    private $png = false;
    private $tableName = 'b_image_compress_files';
    public $LAST_ERROR;
    ...
    public function compressJPG($strFilePath) {
        $res = false;
        if(!$this->isJPEGOptim()){
            $this->LAST_ERROR = Loc::getMessage('IMAGE_COMPRESS_NO_MODULE',array('#MODULE#'=>'jpegoptim'));
            return $res;
        }
    ...
    public function compressPNG($strFilePath) {
        $res = false;
        if(!$this->isPNGOptim()){
            $this->LAST_ERROR = Loc::getMessage('IMAGE_COMPRESS_NO_MODULE',array('#MODULE#'=>'optipng'));
            return $res;
        }
        if(file_exists($strFilePath)) {
            $strFilePath = strtr(
        ...
        if(file_exists($strFilePath)){
            ...
            switch ($arFile["CONTENT_TYPE"]) {
                ...
                case 'image/png' :
                    ...
                    break;
                default :
                    $this->LAST_ERROR = Loc::getMessage('IMAGE_COMPRESS_CONTENT_TYPE',array('#TYPE#'=>$arFile["CONTENT_TYPE"]));
                    return null;
            }

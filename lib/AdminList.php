...
//		$this->file_edit = str_replace('list.php', 'edit.php', $this->file);
		$this->tableID = 'tbl_image_compress_'.substr($this->file, 0, -4);
	...
	public function setContextMenu($arAddContext=array()) {
        ...
        $arContext['compress_all'] = array(
            'TEXT' => Loc::getMessage('COMPRESS_BTN_ALL'),
            'LINK' => '/bitrix/admin/image_compress_files.php?compress_all=Y',
        );
...
//        compress_result=Y&status=success
        if($_REQUEST['compress_result']) {
            if($_REQUEST['status']=='success') {
                \CAdminMessage::ShowMessage(array(
                    "MESSAGE"=> Loc::getMessage('IMAGE_COMPRESS_COMPRESS_IMAGE_STATUS_SUCCESS'),
                    "TYPE"=> "OK",
                ));
            }
        }
...
//                    $recCompress = true;
            ...
            if($recCompress===false) {
                ...
                if(!$msgError) $msgError = Loc::getMessage('IMAGE_COMPRESS_DEFAULT_TEXT_ERRROR');
                \CAdminMessage::ShowMessage(array(
                    "MESSAGE"=> $msgError,
                    "TYPE"=> "ERROR",
                ));
                ...
                $admFields = array(
                    "MESSAGE" => Loc::getMessage('IMAGE_COMPRESS_COMPRESS_IMAGE_PROGRESSBAR'),

...
if ($request->isPost() && check_bitrix_sessid()) {
    ...
    if($request->getPost('test_module')) {
        ...
        if(!Check::isJPEGOptim()) {
            $text[] = Loc::getMessage('IMAGE_COMPRESS_ERROR_CHECK_NOFOUND',array('#MODULE#'=>'jpegoptim'));
        }
        if(!Check::isPNGOptim()) {
            $text[] = Loc::getMessage('IMAGE_COMPRESS_ERROR_CHECK_NOFOUND',array('#MODULE#'=>'optipng'));
        }
        if(!$text) {
            $text = Loc::getMessage("COMPRESS_OPTIONS_TESTED");
        } else {
            $error = true;
            $text = implode (
                PHP_EOL,
                array_merge(
                    array(Loc::getMessage("COMPRESS_OPTIONS_NO_TESTED")),
                    $text
                )
            );
        }
        ...
        CAdminMessage::showMessage(array(
            "MESSAGE" => Loc::getMessage("COMPRESS_REFERENCES_OPTIONS_SAVED"),
            "TYPE" => "OK",
        ));
    }
}
...
<form method="post" action="<?=sprintf('%s?mid=%s&lang=%s', $request->getRequestedPage(), urlencode($mid), LANGUAGE_ID)?>">
    ...
    ?>
<!--    <tr class="heading">-->
<!--        <td colspan="2"><b>--><?//echo GetMessage("COMPRESS_HEADER_SETTINGS")?><!--</b></td>-->
<!--    </tr>-->
    <tr>
        <td width="40%">
            <label for="path_to_jpegoptim">
                <?=Loc::getMessage("COMPRESS_REFERENCES_PATH_JPEGOPTI") ?>:
            </label>
        </td>
        <td width="60%">
            <input type="text"
    ...
    <tr>
        <td width="40%">
            <label for="path_to_optipng">
                <?=Loc::getMessage("COMPRESS_REFERENCES_PATH_PNGOPTI") ?>:
            </label>
        </td>
        <td width="60%">
            <input type="text"
    ...
    <tr>
        <td width="40%">
            <label for="enable_element">
                <?=Loc::getMessage("COMPRESS_REFERENCES_ENABLE_ELEMENT") ?>:
            </label>
        </td>
        <td width="60%">
            <input type="checkbox"
    ...
    <tr>
        <td width="40%">
            <label for="enable_section">
                <?=Loc::getMessage("COMPRESS_REFERENCES_ENABLE_SECTION") ?>:
            </label>
        </td>
        <td width="60%">
            <input type="checkbox"
    ...
    <tr>
        <td width="40%">
            <label for="enable_resize">
                <?=Loc::getMessage("COMPRESS_REFERENCES_ENABLE_RESIZE") ?>:
            </label>
        </td>
        <td width="60%">
            <input type="checkbox"
    ...
    <tr>
        <td width="40%">
            <label for="enable_save">
                <?=Loc::getMessage("COMPRESS_REFERENCES_ENABLE_SAVE") ?>:
            </label>
        </td>
        <td width="60%">
            <input type="checkbox"
    ...
    <tr>
        <td width="40%">
            <label for="jpegoptim_compress">
                <?=Loc::getMessage("COMPRESS_REFERENCES_JPEG_COMPRESS") ?>:
            </label>
        </td>
        <td width="60%">
...
<!--            />-->
    ...
    <tr>
        <td width="40%">
            <label for="enable_element">
                <?=Loc::getMessage("COMPRESS_REFERENCES_JPEG_PROGRESSIVE") ?>:
            </label>
        </td>
        <td width="60%">
            <input type="checkbox"
    ...
    <tr>
        <td width="40%">
            <label for="optipng_compress">
                <?=Loc::getMessage("COMPRESS_REFERENCES_PNG_COMPRESS") ?>:
            </label>
        </td>
        <td width="60%">
    ...
    <input type="submit"
           name="test_module"
           value="<?=Loc::getMessage("COMPRESS_REFERENCES_TEST_BTN") ?>"
    />
    <?/* ?>
    <input type="submit"
           name="restore"
